-- 0013_activity_avg_pace.sql
--
-- Adds the steady-effort average pace to activities.
--
-- The parser has always read `5:50 per km` into its cardio metrics, but the
-- activity had no column for it and — unlike floors, steps and speed — it was
-- never routed into `details` either, so the average pace of every plain run
-- was silently dropped (noticed 2026-09-06). Interval paces were unaffected:
-- they live on cardio_intervals.pace_seconds_per_km, and this column follows
-- the same convention (seconds per kilometre, `5:50` -> 350).
--
-- Nullable like every other activity metric: a missing pace is missing, not
-- zero, and it is never derived from duration/distance.

alter table public.activities
    add column if not exists avg_pace_seconds_per_km numeric;

comment on column public.activities.avg_pace_seconds_per_km is
    'Average pace of the steady effort in seconds per kilometre, as stated in the source (`5:50 per km` -> 350). Null when the source stated none; never derived. Per-interval paces live on cardio_intervals.';

-- apply_import_entry learns the new column. Full replacement of the 0011
-- function body; the only change is avg_pace_seconds_per_km in the activities
-- insert (column list + `avgPaceSecondsPerKm` value).
    ...
    loop
        ...
        loop
            insert into public.activities (
                ...
                duration_seconds, distance_km, avg_pace_seconds_per_km, calories,
                avg_heart_rate_bpm, max_heart_rate_bpm, cadence_spm, elevation_gain_m,
                avg_power_watts, external_load_kg, details, notes, original_text
            )
            values (
                ...
                (v_activity ->> 'avgPaceSecondsPerKm')::numeric,
