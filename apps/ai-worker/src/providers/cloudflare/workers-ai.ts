 ...
 * nothing here assumes a shape: results are validated, and an unreadable result
 * is classified as either *repairable* (bad JSON — worth one retry with a hint)
 * or *upstream* (the binding itself failed, retrying the same call will not
 * help).
 */
